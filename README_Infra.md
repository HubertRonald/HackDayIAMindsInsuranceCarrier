...
- Service Account y permisos IAM para Cloud Run
- Cloud Run (backend)
- Firebase Web App (registro de la aplicación)

> **Precondiciones y despliegues externos a Terraform:** la base Firestore `(default)` debe existir previamente en el proyecto, y Firebase Hosting se publica mediante Firebase CLI desde `deploy-app.yml`.
...
cd terraform
terraform init
terraform apply -auto-approve -var="project_id=MI_PROYECTO"
```
...
```mermaid
flowchart TD
    A["Push a main / releases/** o ejecución manual"] --> B["🚀 deploy-app.yml<br/>Terraform + Backend (Cloud Run) + Frontend (Firebase Hosting)"]
...
|----------------------------|------------------------------|-----------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
| `make deploy-actions`      | `.github/workflows/deploy-app.yml` | 🚀 Crea/actualiza la infraestructura con Terraform + despliega **backend (Cloud Run)** + **frontend (Firebase Hosting)** usando la rama actual. | Cuando quieres desplegar tu aplicación y la infraestructura en la nube sin correr nada localmente. Ideal para pruebas o despliegue manual. |
| `make populate-actions`    | `.github/workflows/populate-data.yml` | 📦 Genera datos dummy y los carga en **BigQuery** y **Firestore** (requiere infraestructura Terraform desplegada y Firestore `(default)` preexistente). | Después de un despliegue para poblar datos de prueba sin correr scripts locales.   |
...
```mermaid
flowchart TD
    A[💻 Developer ejecuta 'make deploy-actions'] -->|1️⃣ Llama API de GitHub| B[GitHub Actions Workflow: deploy-app.yml]
    B -->|2️⃣ Terraform Apply| C[GCP Infraestructura: Artifact Registry, BigQuery; Firestore es precondición externa]
